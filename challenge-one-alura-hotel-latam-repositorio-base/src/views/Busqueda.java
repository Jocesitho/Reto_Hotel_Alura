...
public class Busqueda extends JFrame {
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(filaReservas>=0) {
					...
					int confirmarH=JOptionPane.showConfirmDialog(null,"Desea borrar la huesped?");
					
					if(confirmarH==JOptionPane.YES_OPTION) {
						String valor=tbHuespedes.getValueAt(filaHuespedes,0).toString();
						huespedesControl.EliminarHuespedes(Integer.valueOf(valor));
						...
						JOptionPane.showMessageDialog(null,"Hubo un error al eleminar");
				}
			}
		});
	...
	private void ActualizarReservas() {
		...
		.ifPresent(fila->{
			...
			if(tbReservas.getSelectedColumn()==0) {
				JOptionPane.showMessageDialog(this,"No es posible editar el ID");
			}else {
				this.reservasControl.actalizarReserva(dataE, dataS, valor, formaPago, id);
	...
	private void actualizarHuespedes() {
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(),
				tbHuespedes.getSelectedColumn())).ifPresentOrElse(filaHuespedes->{
					...
					if(tbHuespedes.getSelectedColumn()==0 || tbHuespedes.getSelectedColumn()==6) {
						JOptionPane.showMessageDialog(this,"No se pude modificar los ID");
					}else {
						
						this.huespedesControl.ActualizarHuespedes(nombre, apellido, fechaNacimiento, nacionalidad, telefono, id_reserva, id);
						JOptionPane.showMessageDialog(this,String.format("Registro modificado con exito"));
					}

				
				},()->JOptionPane.showInternalMessageDialog(this,"Hubo un error"));
	}
